...
//    		imgPath = upload.files[0].getAsDataURL();
    		...
    		document.getElementById(divImage).innerHTML = "<img id='detailimg' src='"+imgPath+"' width='"+width+"' height='"+height+"'/>";
	}else{
    	//IE浏览器
		//先去掉本来要显示的照片
...
<div id="preview"
	...
	<img id="detailimg"
		<c:if test="${empty  productSale.product.smallImageUrl}">src=# </c:if>
		<c:if test="${not empty productSale.product.smallImageUrl}">src=${productSale.product.smallImageUrl} </c:if>
		width="80" height="80">
</div>
