				...
				<form action="" >
					<table  class="list-table">
						...
						<tr>
							...
							<td colspan="3"> <fmt:formatDate value="${promotion.startDate }" type="date"/>
						...
						<tr>
							...
							<td colspan="5"> ${promotion.description }</td>
						</tr>
						<tr>
							<td align="right">促销语：</td>
							<td colspan="5"> ${promotion.advert }</td>
						</tr>
						<!-- 促销方式 -->
						<tr>
							<td align="right">促销方式：</td>
							<td colspan="5">${promotion.manner.name}
							</td>
						</tr>
						</table>
						<hr />
						<table class="list-table">
							...
							<c:forEach items="${promotion.orderRules}" var="rule">
								<tr>
									<td>订单购买指定商品   满  ${rule.minAmount } 元    省  ${rule.amount } 元</td>
								</tr>
							</c:forEach>
						</table>
						<hr />
						<table class="list-table" id="productlist">
							<caption>参与活动的商品</caption>
							<tr>
								<th>商品编码</th>
								<th>商品名称</th>
								<th>商品类别</th>
