			...
			<div class="frame-main-inner" id="content">
				...
				<div id="add-body">
					...
					<form:form commandName="shopForm" name="shopForm" id="shopForm"  enctype="multipart/form-data"
						action="${contextPath}/seller/_update" method="post">
						...
						<div>
							<table class="list-table1">
								...
								<tr>
									...
									<td align="left"><input type="password" name="passWrod" id="passWrod" value="${shopForm.passWrod}"  /><input type="hidden" value="${pwd}" name="pwd"/>
								...
								<tr>
									...
									<c:forEach items="${storage.children }" var="storage" varStatus="s">
										<c:if test="${storage.id==storageType }">
											<form:option value="${storage.id }" selected="selectedd">${storage.name }</form:option>
										</c:if>
										<c:if test="${storage.id!=storageType }">
								...
								<tr>
									...
									<td align="left"><fmt:formatDate type="both" value="${openTime}"/></td>
								</tr>
								<tr>
									<td align="right">激活时间：</td>
									<td align="left"><fmt:formatDate type="both" value="${activeTime}"/></td>
								</tr>
								<tr>
						...
						<div style="float: right; margin-top: -100px; margin-right: 50px;">
							<div id="preview" 
								...
								<img id="detailimg" 
								<c:if test="${empty  shopForm.logo }">src=/imgs/seller/default.jpg </c:if> <c:if test="${!empty  shopForm.logo }">src=http://static.winxuancdn.com/upload/seller/${shopForm.logo} </c:if>  
