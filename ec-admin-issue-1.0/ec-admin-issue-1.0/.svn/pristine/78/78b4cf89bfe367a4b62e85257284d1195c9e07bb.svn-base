...
<body>
	<div class="frame">
		...
		<div class="frame-main">
			<div>
				<form id="exForm" name="exForm" method="POST" target="_blank" action="/excel/channelPayDetailTemplate">
				<input type="hidden" value="xls" name="format">
				<a onclick="document.exForm.submit()">到款明细模板下载</a>
				</form>
				<form name="form" action="/channel/pay/uploadDetail"  method="post" enctype="multipart/form-data">
					<label>文件:</label>
					<input name="file" id="file" type="file" onchange="javascript:document.getElementById('upload').style.display = '';" />
					<input type="button" style="display:none" id="upload" value="上传明细" onclick="uploadPayDetail(this);"/>
				</form>
				<!-- <iframe name="upload" style="border:0px;width: 100%;height: 500px;"></iframe> -->
				<hr />
				<ul>
					<c:forEach items="${histories }" var="h" varStatus="s">
						<li <c:if test="${s.index == 0}"> style='color:red' </c:if> >
							<fmt:formatDate value="${ h.uploadtime}" type="both"/>
							　上传人:${h.employee.name }
							　记录条数:${h.total }
							　文件名:${h.filename }
