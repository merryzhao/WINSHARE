...
<body>
		...
		<div class="frame-main">
			...
			<div class="frame-main-inner" id="content">
				<div id="content-result">
					<c:choose>
						<c:when test="${!empty orderShipperCounts}">
							<h4>未审核通过订单列表</h4>
							...
							<table class="list-table">
								...
								<c:forEach var="order" items="${orders}"
									...
									<tr class="${rowStatus}">
										...
										<td align="left">${order.channel.name}</td>
										<td>${order.purchaseQuantity}</td>
										<td>${consignee.city.name}&nbsp;${consignee.district.name}</td>
										<td align="left">${consignee.address}</td>
										<td align="left">${consignee.remark}</td>
										<td align="left">
										<c:if test='${!empty errorMessage[orderId]}'>
											${errorMessage[orderId]}
										</c:if>
										</td>
										<td><a  class="operate-link"
											href="javascript:void(0);" onclick="confirmSingle(${order.id});">承运商审核</a>
										</td>
									</tr>
								</c:forEach>
								<tr>
	...
	<div id="confirmDiv2" align="center">
		...
		<form id="">
			<select name="deliveryCompany">
				<option value="-1">选择承运商</option>
				<c:forEach var="deliveryCompany" items="${deliveryCompanys}">
	...
	<script type="text/javascript"
		src="${contextPath}/js/order/auditOrderShipper.js"></script>
</body>
</html>