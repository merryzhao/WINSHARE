...
function validateCategoryPrice(){
					if($("#category_price").find(".sellers").text()==""){
						 alert("请选择卖家");
						 return false;
					 }
					if($("#category_price").find(".discount_method").length<1){
						 alert("请选择分类");
						 return false;
					}
					var flag = true;
					$("#category_price").find(".discount").each(function(){
						var   newPar=/^\d+(\.\d+)?$/
						if(!newPar.test($(this).val())){
							$(this).focus();
							$(this).attr({"title":"折扣必须为整数"});
							flag=false;
							return false;
						}
						if($(this).val()==""){
							$(this).focus();
							$(this).attr({"title":"请填写折扣"});
							flag=false;
							return false;
						}
					...
					$("#categorylist tbody tr:last")
							.after(
									"<tr class=selectData><td><input type=checkbox name=chk /><input type=hidden name=categorys value="+id+" /></td><td>"
											+ string
											+ "</td><td><select name=categoryDisTypes class=discount_method ><option value=26001>统一折扣</option><option value=26002>只下调折扣</option><option value=26003>只上调折扣</option></select></td><td><input name=categoryDiscounts type=text class=discount size=5 /></td><td><a href=javascript:void(0) class=del>删除</a></td></tr>");
				}
			}
		...
		$("#change_discount").click(function() {
			$("input[name=chk]:checked").each(function() {
				...
				discount_method.val($("#discount_method").val());
