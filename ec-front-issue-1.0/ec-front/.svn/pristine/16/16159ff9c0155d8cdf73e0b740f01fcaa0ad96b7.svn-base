...
<c:choose>
  ...
  <link rel="stylesheet" href="${miscServer}/cms/assets/styles/wx-www/cp-signin-ext.css"/>
  <!--[if lte IE 6]>
...
<!-- 悬浮表头 结束 -->
<div class="header">
	<h1><p class="service-tel">客服热线：400-702-0808</p><img src="${miscServer}/css2/images/logo.png"/></h1>
	<div class="progress${progressClass}"></div>
</div>
<div class="content">
	<!-- 购物车商品列表 开始 -->
	<div class="table-list" id="cart-list">
		...
		<div class="group" data-shop-id="${seller.key.shop.id}" data-supply="${seller.key.supplyType.id}">
						...
						<c:if test="${prom.supplyTypeCode==seller.key.supplyType.id&&prom.shopId==seller.key.shop.id}">
							<c:choose>
								...
								<c:otherwise>
									...
									<c:if test="${prom.promotionPrice.promType==20008}">
										<c:set var="saveProm" value="${prom}"/>
									</c:if>
								</c:otherwise>
							</c:choose>
			...
			<c:set var="state" value='${(item.statusCode!=13002)?" out-of-stock":""}'/>
			<div class="trow${border}${state}">
				<c:choose>
					<c:when test="${item.statusCode==13002}">
						<div class="checkbox"><input name="item" type="checkbox" value="${item.productSaleId}"/></div>
						<div class="product">
							<a href="${item.url}" target="_blank"><img src="${item.imageUrl}" alt="${item.sellName}" /></a>
							<a href="${item.url}" target="_blank" title="${item.sellName}">${item.sellName}</a>
						</div>
					...
					<c:when test="${item.statusCode==13001}">
						...
						<div class="operation"><a href="javascript:;" bind="arrival_notice" data-id="${item.productSaleId}">到货通知</a><a href="javascript:;" bind="remove" data-id="${item.productSaleId}">删除</a></div>
					</c:when>
					<c:otherwise>
						<div class="checkbox"><input name="item" type="checkbox" value="${item.productSaleId}" disabled="disabled"/></div>
						<div class="product">
						...
						<div class="credits">--</div>
						<div class="privilege">&nbsp;</div>
						<div class="quantity"><em data-id="${item.productSaleId}">此商品暂停销售</em></div>
						<div class="operation"><a href="javascript:;" bind="remove" data-id="${item.productSaleId}">删除</a></div>
					</c:otherwise>
				</c:choose>
			</div>
		...
		<div class="tfoot">
			...
			<div class="count">
				...
				<div class="balance">
					...
					<label>合计（不含运费）</label>
					<p class="price">&yen;<em>${shoppingcart.salePrice-shoppingcart.saveMoney}</em></p>
				</div>
			</div>
			<%-- 结算信息结束  --%>
		</div>
	...
	<%-- 购物车商品列表 结束 --%>
	<!-- 悬浮结算信息 开始 -->
	<div class="atf-balance" style="display:none;">
		<ul>
		<li class="brief">
			<div>
				<span>共<b>${shoppingcart.count}</b>件商品</span>
				<label>总计：</label>
				<p class="price">&yen;<em>${shoppingcart.salePrice}</em></p>
			</div>
			<div class="save">
		...
		<li><a class="balance"><i>去结算</i><b></b></a></li>
		</ul>
	</div>
	<!-- 悬浮结算信息 结束 -->
	<div class="button-wrap">
		<p class="valid-tip">温馨提示：请您先删除购物车中的“已下架”或“暂停销售”的商品，方可继续结算！</p>
		<a class="continue"><i>继续购物</i><b></b></a>
		<a class="balance"><i>去结算</i><b></b></a>
	</div>
...
<c:choose>
<c:when test="${isRelease}">
	<script src="${jsPrefix}/??lib/seajs/1.3.0/sea.js,etc/seajs-config.js,bin/mini-atf.js?${version}" data-main="${jsPrefix}/app/shoppingcart.js?${version}" id="seajsnode"></script>
</c:when>
<c:otherwise>
	<script src="${jsPrefix}/lib/seajs/1.3.0/sea.js"></script>
	<script src="${jsPrefix}/etc/seajs-config.js"></script>
	<script src="${jsPrefix}/bin/mini-atf.js"></script>
	<script>
		seajs.use("${jsPrefix}/app/shoppingcart");
	</script> 
</c:otherwise>
</c:choose>
<script src="${miscServer}/js/o_code.js?${version}"></script>
<script>
(function() {
