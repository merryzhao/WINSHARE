...
<div fragment="${fragment.id}" content="bookshop" style="display:none;">
...
<jsp:include page="../null.jsp"/>
	<c:forEach items="${contentList}" var="content" varStatus="status">
		<li><a class="pro_pic"   target="_blank"  href="${content.url }"><img	src="${content.imageUrl }" alt="${content.product.name }">
			...
			<p>
				<a   target="_blank"  href="${content.url }" title="<c:out value="${content.name }"/>"><winxuan-string:substr length="15" content="${content.name}"></winxuan-string:substr> </a>
			</p> 
