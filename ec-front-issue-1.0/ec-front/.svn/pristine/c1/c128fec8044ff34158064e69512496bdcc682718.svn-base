...
<div class="tabcontent01"> 
	<table class="book_more" width="90%" border="0" cellspacing="0"
		cellpadding="0">
		<tr>					
			<c:set value="0" var = "i"/>
            <c:forEach var="extend" items="${product.extendList}" varStatus="status">
            	<c:if test="${extend.show && extend.productMeta.open.id != 82002}">
            	<c:if test="${fn:trim(extend.value)!='' && extend.value!='无' && extend.value!='' && extend.value!='NULL' && extend.value!='null' }">
                   <td> 
                   		<c:if test="${extend.productMeta.id == 142 }">其他著作者：${extend.value}</c:if>
                    	<c:if test="${extend.productMeta.id != 142 }">${extend.name=='ISBN'||extend.name=='isbn'?'ISBN': extend.name} : ${extend.value }</c:if>
           
						<c:if test="${extend.name=='重量' && regPureNum}">&nbsp;g</c:if>
		...
		<tr>
			<td class="book_cate" colspan="2"></td>
		</tr>

	</table>
	
	<table class="book_category" width="90%" border="0" cellspacing="0"
		cellpadding="0">
		<c:forEach var="category"
					items="${product.categories}" varStatus="status">
           <tr>
             <td class="book_category_width"> 
             	<c:if test="${status.index==0}">
                  	商品分类:
             	</c:if>
             </td>
		        <td>${category.allCategoryName}</td>
		</tr>
		</c:forEach>
		</table>
	 <cache:fragmentPageCache idleSeconds="86400"  key="FRAGMENT_3002"> 
        	<c:import url="/fragment/3002">
...
<div class="tabcontent02">
	...
	<ul class="inner-blend-tab">
		...
		<li index="1" data-ga="4">天猫评论(${tmallCommentCount})</li>
	</ul>
	<script>
		seajs
				.use(
	...
	<div id="comments-winxuan">
		<div class="commnet_score">
			<div class="rating_results">
				<p>
					用户评分：<a class="rating_link" href="#" id="avgStarScore">${aveRank}</a>星（共<a
