	...
	function ToolKit(cfg){
		this.opt={
			button:{
				...
				reduce:"[bind='reduce']",
				increase:"[bind='increase']"
			},
			context:$(document),
			domain:document.domain
		};
		this.buttons=[];
		this.pricecutUrl=conf.portalServer+"/customer/customerinfostatus?format=json&t="+new Date().getTime();
		$.extend(this.opt,cfg);
		this.init();
	};
	ToolKit.prototype={
		...
		bind:function(){
			...
			$(this.buttons).click(function(e){
				...
				if(typeof __ozclk != 'undefined' && __ozclk instanceof Function)__ozclk();//99click监控
				e.preventDefault();
				e.stopPropagation();
				return false;
			});
		...
		pricecutNotify:function(obj){
		    var el=$(this);
		    $.getJSON(obj.pricecutUrl,function(data){
                if(data.isLogin){ 
                    if(!obj.notifyWin||obj.notifyWin.hasClosed){
                        obj.notifyWin=ui.priceCut({pid:el.data("id"),price:el.data("price"),email:data.email,phone:data.phone});
                    }
                }else{
                    obj.login(el,obj.pricecutNotify);
                }
		...
		reduce:function(){
		    var el=$(this).siblings("input.buy-num-text"),num=parseInt(el.val());
		    if(!!num&&num>1){
		        num--;
		    }
		    el.val(num);
