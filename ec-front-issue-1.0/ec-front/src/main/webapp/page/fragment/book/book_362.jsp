	...
	<ol>
		<c:forEach items="${contentList}" var="content" varStatus="status">
			<c:if test="${status.first }">
				<li class="first"><span class="no1 fl"></span><a title="<c:out value="${content.name }"/>"		  target="_blank"  href="${content.url }">
			...
			<c:if test="${!status.first }">
				...
				<a title="<c:out value="${content.product.name }"/>"		  target="_blank"  href="${content.url }"><winxuan-string:substr length="16" content="${content.product.name }"/></a></li>
			</c:if>
		 </c:forEach> 
		<li class="txt_right">
	<%--
