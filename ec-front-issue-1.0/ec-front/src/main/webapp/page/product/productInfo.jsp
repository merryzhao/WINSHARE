<%@page pageEncoding="UTF-8"%><%@include file="../snippets/tags.jsp"%>
        ...
        <c:if test="${!empty alsoBoughtList}">
        	<div class="also_purchased">
            	...
            	<%-- <a id="left_roll" href="javascript:;"></a> <a id="right_roll" href="javascript:;"></a> --%>
            	<div class="purchase_box">
                	<ul id="recom_goods_scroll_ul_thing_item">
                   		<c:forEach var="productBought" items = "${alsoBoughtList}" varStatus="status">
                    		<li>
                        		<div class="goods_picture"><a href="${productBought.product.url }" title="${productBought.effectiveName }"><img alt="${productBought.effectiveName}" src="${productBought.product.imageUrlFor110px}"></a></div>
                        		<strong><a class="link4" href="${productBought.product.url }" title="${productBought.effectiveName}">${productBought.effectiveName}</a></strong> 
                        		<span class="goods_price"><b class="red fb">￥${productBought.salePrice }</b> <del>￥${productBought.product.listPrice }</del></span>
                    		</li>
                   		</c:forEach>
                	</ul>
           		</div>
        ...
        <ul class="infor_tab">
            ...
            <li target="tabcontent02" data-ga="1">商品评论（${commentCount}条）</li>
            <li target="tabcontent03" data-ga="2">商品咨询（<b class="questionCount" style="font-weight:700;">${questionCount}</b>条）</li>
        </ul> 
        <div class="tabcontent01">
            <table class="book_more" width="90%" border="0" cellspacing="0" cellpadding="0">
            	<tr>
                	<c:forEach var="extend" items="${product.extendList }" varStatus="status">
                    	<td> ${extend.name=='ISBN'||extend.name=='isbn'?'条形码': extend.name} :${extend.value }</td>
            ...
            <c:forEach var="description" items="${descriptions}" >  
            	<c:if test="${!empty description.content && description.content!='null'}">                
           			<div class="moreinfo_box">           
                    	<h2><b class="info_title">${description.name}</b></h2>     
                       	<div class="morereader">
                       		<div class="content">
                       		<c:if test="${product.sort.id==11003 }">
								${description.content}
							</c:if>
							<c:if test="${product.sort.id!=11003 }">
                        ...
                        <c:if test="${description.productMeta.id == 9 }">
                        	<c:if test="${!empty newSaleList}">       
              			 		<dl class="hot_new">
                  		   			<dt><a class="link4" href="#">热门新书</a></dt>
                  		   			<c:forEach var="newSale" items="${newSaleList}" varStatus="status">
                  		    			<dd><a href="${newSale.product.url }" title="${newSale.product.name }"><winxuan-string:substr length="25" content="${newSale.product.name }"></winxuan-string:substr></a></dd>
                  		   			</c:forEach>
                            		<dt class="hei1"></dt>
               		       		</dl>
               		      	</c:if>
               		     ...
               		     <c:if test="${description.productMeta.id == 10 }">
                         	<p style="display:none;">此书可以试读，请<a class="link3" href="javascript:;">点击这里&gt;&gt;</a></p>
                         </c:if> 
           			 </div>
           		</c:if>
         	</c:forEach>
        ...
        <div class="tabcontent02">
            <h4 class="comment_tit">商品评论（${productSale.performance.totalComment}条）</h4>
            <div class="commnet_score">
                <div class="rating_results">
                    <p>用户评分：<a class="rating_link" href="#" id="avgStarScore">${avgRank}</a>星（共<a class="rating_link" href="#" id="allScoreCount">${rankCount}</a>人参与）</p>
                    <ul>
                        <li>
                    ...
                    <ul>
                    	<li id="login_info">
							<c:if test="${not empty customer }">
								<p class="comm_account">用户：<span class="link1" id="current_user">${customer.name}</span>已登录 
								<a class="link1" href="javascript:;" bind="login">更换账户&gt;&gt;</a></p>
							</c:if>
						</li>
                        <li><div class="do_score" id="do_score" param="1"><span class="fb f14">我来评分：</span><b class="star1"></b><b class="star2"></b><b class="star3"></b><b class="star4"></b><b class="star5"></b><label bind="scoring"></label></div></li>
                        <li><a class="w_review" href="/product/${productSale.id }/comment/_new">我来写评论</a><a target="_bland" class="link4" href="http://www.winxuan.com/help/score.html">写点评 送积分 中奖券&gt;&gt;</a></li>
                    </ul>
                </div>
            </div>
            <div class="comment_content">    
                ...
                <p class="txt_center">以下仅展示部分评论，查看所有<span class="red fb">${commentCount}</span>评论请<a class="link1" href="/product/${productSale.id}/comment/list">点击这里&gt;&gt;</a></p>
                <c:forEach var="customerComment" items="${customerComments}"
           		...
           		<h5 class="view_all"><a href="/product/${productSale.id}/comment/list">查看所有${commentCount}条评论&gt;&gt;</a></h5>
                </c:when>
                <c:otherwise>
                	 <p class="txt_center" tyle="height:100px;">暂无评论，想成为第一个评论员吗？有2倍积分哦，<a class="link1" href="/product/${productSale.id }/comment/_new" >立即发表评论>></a></p>
                </c:otherwise>
                </c:choose>               
            </div>
        </div>
        <div class="tabcontent03">
			<h4 class="comment_tit"><a class="fr" href="/product/${productSale.id}/question/list">查看所有<b class="questionCount">${questionCount}</b>条咨询&gt;&gt;</a>商品咨询（<b class="questionCount">${questionCount}</b>条）</h4>
            <div class="comment_content">
					...
					<div class="comment_list">
						...
						<c:if test="${not empty  customerQuestion.replyList}">
							<c:forEach var="reply" items="${customerQuestion.replyList}">
								<div class="reply_item"><h4><c:choose>
