 ...
 */
public class ErpOrderRowMapper implements RowMapper<ErpOrder>{
	...
	public ErpOrder mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		erpOrder.setReturnTypeId(rs.getString("gkthfsid"));
		erpOrder.setDc(rs.getString("dc"));
		try {
			if(!StringUtils.isNullOrEmpty(rs.getString("FHRQ"))){
				SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
				erpOrder.setFhrq(dateFormat.parse(rs.getString("FHRQ")));
			}else{
